...
function rmse = LPV_MPC( Q, xmean, xstdev, ymean,ystdev, mod )
...
% LPV_MPC_controller = LPV_MPC_fnc_dt_Vnew( Hp, SchedVars_Limits, nx, nu );
LPV_MPC_controller = LPV_MPC_fnc_dt_Vnew( Hp, SchedVars_Limits, nx, nu, q1, q2, q3, q4, q5, r1, r2 );  
...
%     V = pred([XX(4,1); XX(5,1);UU(2,1);UU(1,1);Wz(1)] , xmean, xstdev, ymean,ystdev, mod);  %use this when using : best_net/test_net
    V = pred([Vx_ref(i);Vy_ref(i);Accel_ref(i);Steer_ref(i);Omega_ref(i);cf(i);cr(i)] , xmean, xstdev, ymean,ystdev, mod);  %use this when using elma_net
    cf(i+1) = abs(V(1));
    cr(i+1) = abs(V(2));
    ...
    if i == 1      
        ...
        for j=1:Hp 
            [ A_vec(:,:,j), B_vec(:,:,j), cf(j), cr(j)  ] = Discrete_AB_Comp ...
        ...
        for j=1:Hp
            ...
            [ A_vec(:,:,j), B_vec(:,:,j), cf(i+1+j-1), cr(i+1+j-1)] = Discrete_AB_Comp ...
    ...
    [T,x] = ode45(@(t,x) nonlinear_Berkeley_model(t,x, [steer; accel; Curv_ref(i); V_wind; Road_slope]), T, vehicle_states);  
